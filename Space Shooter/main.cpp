...
public:
	...
	void InitEnemy(Vector3 position) {
		*this->instance = rocket[0];
		this->instance->SetPosition(position);
		this->instance->SetAcceleration(Vector3::Down);
		this->instance->SetColor(this->color[RANDOM_COLOR]);
		//this->instance->SetScale(Vector3::One * 0.5f);
		//this->instance->SetColliderSize(Vector3::One * 0.5f);
	...
	void EnemyExplode() {
		this->instance->SetAcceleration(Vector3::Zero);
		this->instance->SetVelocity(Vector3::Zero);
...
public:
	...
	void ShowTimer() {
		if(!this->hasSpawned && tmpTime > 0) {
			...
			AddText2D(this->spawnerPos.X - 0.5f, this->spawnerPos.Y - 1.5f, text, FontSize::MEDIUM_FONT);
		}
	}
	...
	void SetSpawnerPosition(Vector3 pos) {
		this->spawnerPos = pos;
	}
...
public:
	Bullet() { 
		this->posOffset = Vector3::Up * 0.5f;
		this->yLimit = 8.0f;
	}
	...
	void SpawnBullet(Vector3 position, Color3 color) {
		GameObject* tmpBullet = new GameObject;
		tmpBullet->SetPosition(position + posOffset);
		tmpBullet->SetColor(color);
		tmpBullet->SetScale(Vector3::One * 0.2f);
		tmpBullet->SetColliderSize(Vector3::One * 0.2f);
		tmpBullet->SetActiveCollider(false);
		tmpBullet->SetVelocity(Vector3::Up);
...
public:
	...
	void OnClick() {
		...
		if(!this->isMine && 
			...
			this->instance->GetColor() == this->color[this->clickCount]) * lifeColorOffset[this->life]) return;
		...
		this->instance->SetRotation(this->curRot, Vector3::Up);
	}
...
void InitMainGame() {
	...
	for(int i = 0; i < turretCount[optionIndx]; i++) {
		...
		tmpTurret->TurretInstance()->SetPosition(Vector3((i - turretPosX[optionIndx]) * 3.5f, -7.0f, 0.0f));
		tmpTurret->TurretInstance()->SetScale(Vector3::One * 0.5f);
		tmpTurret->TurretInstance()->SetColliderSize(Vector3::One * 3.0f);
		tmpTurret->TurretInstance()->SetActiveCollider(false);

		turret.push_back(tmpTurret);

		EnemySpawner* tmpSpawner = new EnemySpawner;
		tmpSpawner->SetSpawnerPosition(Vector3((i - turretPosX[optionIndx]) * 3.5f, 8.0f, 0.0f));
...
void Start() {
	...
	// Start Screen
	addButton->SetPosition(Vector3(1.5f, -1.0f, 0.0f));
	addButton->SetRotation(-90, Vector3::Front);
	addButton->SetScale(Vector3::One * 0.2f);
	addButton->SetActiveCollider(false);

	subButton->SetPosition(Vector3(-1.5f, -1.0f, 0.0f));
	subButton->SetRotation(90, Vector3::Front);
	subButton->SetScale(Vector3::One * 0.2f);
...
void Update() {
	...
	if(gameMode == StartScreen) {
		...
		if(showTitle)
			AddText2D(-1.6f, 1.0f, "Space Shooter", FontSize::LARGE_FONT);

		if(CheckRayHit(*addButton)) {
			optionIndx++;
		...
		AddText2D(-0.9f, -0.5f, "Game Mode", FontSize::MEDIUM_FONT);
		AddText2D(textPosX, -1.15f, options[optionIndx], FontSize::MEDIUM_FONT);
		AddText2D(-1.7f, -2.5f, "Press spacebar to START!", FontSize::MEDIUM_FONT);
	...
	if(gameMode == MainGame) {
		...
		if(!turret.empty()) {
			...
			for(int i = 0; i < turret.size(); i++) {
				//if(turret.size() >= 4)
				//	turret[i]->TurretInstance()->SetColliderSize(Vector3::One * 2.5);
				//else
				//	turret[i]->TurretInstance()->SetColliderSize(Vector3::One);
		...
		AddText3D(-10.0f, 7.0f, 1.0f, tmpText, FontSize::MEDIUM_FONT);

		sprintf(tmpText, "Wave %i", wave);
		AddText3D(-0.5f, 7.0f, 1.0f, tmpText, FontSize::MEDIUM_FONT);

		sprintf(tmpText, "Score %i", score);
		AddText3D(8.0f, 7.0f, 1.0f, tmpText, FontSize::MEDIUM_FONT);
	}
	...
	if(gameMode == CreditScreen) {
		...
		if(textIndx == 0)
			AddText2D(-0.95f, -0.5f, "GAME OVER!", FontSize::MEDIUM_FONT);
		if(textIndx == 1) {
			sprintf(tmpText, "Final Score: %i", score);
			AddText2D(-0.95f, -0.5f, tmpText, FontSize::MEDIUM_FONT);

			sprintf(tmpText, "Maximum Waves: %i", wave);
			AddText2D(-1.35f, -1.0f, tmpText, FontSize::MEDIUM_FONT);
		}
		if(textIndx == 2) {
			AddText2D(-2.0f, -0.5f, "Created by: Anthony Ganzon", FontSize::MEDIUM_FONT);
			AddText2D(-0.9f, -1.0f, "PROJECT K3AI", FontSize::MEDIUM_FONT);
		}
		if(textIndx == 3) {
			AddText2D(-1.5f, -0.5f, "Thank you for playing!", FontSize::MEDIUM_FONT);
			AddText2D(-1.0f, -1.0f, "Press Esc to Exit", FontSize::MEDIUM_FONT);
		}
	}
	else
		textSwitchTime->LocalReset();
...
void MouseDown(int x, int y) {
	if(GetMouseDown(MouseType::Left)) {
		for(int i = 0; i < turret.size(); i++) {
	...
	if(GetMouseDown(MouseType::Right)) {
		for(int i = 0; i < turret.size(); i++) {
			turret[i]->SetControl(false);
